...
function ListPlaylistOnClick({ item, setOnListen, currentTrack }) {
  return (
    <div className="overflow-y-auto p-6 900:p-10 ">
      <h1 className="text-white font-scada text-3xl font-bold mb-2">PlaylistName</h1>
      {item.map((music, key) => (
        <button key={key} onClick={() => setOnListen(item[currentTrack].s3_link)} type="button" className="bg p-1 w-full">
